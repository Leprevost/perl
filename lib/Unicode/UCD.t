...

{
    ...
    if (ok(defined $r1, "Found Latin script")) {
        my $n1 = @$r1;
        is($n1, 30, "number of ranges in Latin script (Unicode 6.1.0)") if $::ASCII;
        shift @$r1 while @$r1;
...
if ($::ASCII) { # On EBCDIC, other things will come first, and can vary
...
elsif ($::EBCDIC) { # Relies on the ranges 0-9, a-f, and A-F each being
                    # contiguous
    @invlist = prop_invlist("AHex");
    is_deeply(\@invlist, [
            utf8::unicode_to_native(0x0061), utf8::unicode_to_native(0x0066) + 1,
...
foreach my $prop (keys %props) {
            ...
            || $name eq "dm"))
    {
        ...
        elsif ($::EBCDIC) { # Won't work for POSIX-BC
